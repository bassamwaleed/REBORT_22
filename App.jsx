...
export default function App() {
  ...
  // مصفوفات لتوليد الرحلات العشوائية
  const CITIES = ['القاهرة', 'الإسكندرية', 'المنصورة', 'طنطا', 'بنها', 'الزقازيق', 'دمنهور', 'شبين الكوم', 'الاسماعيلية'];
  const RANDOM_NAMES = ['أحمد مجدي', 'سيد عبد الرحمن', 'مريم طارق', 'خالد حسن', 'نورهان جمال', 'مصطفى عادل'];
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        if (currentUser.isAnonymous) {
          ...
          setIsGuest(false);
          // الفحص الأوتوماتيكي للأدمن عن طريق الإيميل
  ...
  // دالة توليد رحلة عشوائية للبوت
  const generateRandomTrip = async () => {
    ...
    try {
      await addDoc(collection(db, APP_COLLECTION_NAME), randomTrip);
      triggerToast('تم توليد الرحلة بنجاح! 🤖');
    } catch (e) {
      alert("حدث خطأ في توليد الرحلة");
    }
  };
  ...
  const renderStars = (rating = 0, total = 0) => {
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400">({parseFloat(rating).toFixed(1)})</span>
      </div>
    );
  };
  ...
  return (
    <div dir="rtl" className={`min-h-screen flex flex-col relative transition-colors duration-300 ${bgMain}`}>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[80] flex justify-center items-center p-4">
          <div className={`${bgModal} rounded-3xl w-full max-w-2xl h-[85vh] shadow-2xl border flex flex-col ${isDarkMode ? 'border-amber-500/30' : 'border-amber-400'}`}>
            ...
            <div className={`flex-1 overflow-y-auto p-6 space-y-8 ${isDarkMode ? 'bg-slate-900' : 'bg-slate-50'}`}>
              ...
              {/* قسم البوت لتوليد النشاط */}
              <div className={`p-5 rounded-2xl border ${bgCard}`}>
                <h3 className="font-bold text-lg mb-2 flex items-center gap-2"><Settings size={18} className="text-amber-500 animate-spin"/> توليد نشاط وهمي (بوت)</h3>
                <p className="text-xs text-slate-500 mb-4">اضغط لتوليد رحلات عشوائية. ستظهر للمستخدمين بحالة "متاحة"، وتتحول تلقائياً لـ "في الطريق" و"مكتملة" بمرور الوقت.</p>
                <button onClick={generateRandomTrip} className="w-full bg-slate-900 text-white dark:bg-slate-700 py-3 rounded-xl font-bold flex justify-center items-center gap-2 hover:bg-slate-800 transition-colors">
                  <Play size={18}/> توليد رحلة عشوائية الآن
                </button>
              </div>

              {/* قسم إدارة المستخدمين (التوثيق) */}
              <div className={`p-5 rounded-2xl border ${bgCard}`}>
                  ...
                  <div key={trip.id} className={`p-5 rounded-2xl shadow-sm border relative flex flex-col ${bgCard}`}>
                    <div className="flex justify-between items-start mb-4">
                      <span className={`text-xs font-bold px-3 py-1.5 rounded-lg ${trip.type === 'offer' ? (isDarkMode ? 'bg-emerald-900/30 text-emerald-400' : 'bg-emerald-100 text-emerald-800') : trip.type === 'delivery' ? (isDarkMode ? 'bg-purple-900/30 text-purple-400' : 'bg-purple-100 text-purple-800') : (isDarkMode ? 'bg-amber-900/30 text-amber-400' : 'bg-amber-100 text-amber-800')}`}>
                        {trip.type === 'offer' ? 'أنا سائق' : trip.type === 'delivery' ? 'دليفري/شحن' : 'أنا راكب'}
                      </span>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-6 w-full pb-28 md:pb-6">
        ...
        <div className={`flex gap-2 p-1.5 mb-8 w-full max-w-2xl mx-auto rounded-2xl shadow-inner overflow-x-auto custom-scrollbar ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
          ...
          <button onClick={() => setFilterType('offer')} className={`flex-1 min-w-[110px] py-2.5 text-sm font-bold rounded-xl transition-all shadow-sm ${filterType === 'offer' ? 'bg-emerald-600 text-white' : (isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-white text-slate-500 hover:bg-slate-50')}`}>أصحاب سيارات 🚗</button>
          <button onClick={() => setFilterType('request')} className={`flex-1 min-w-[110px] py-2.5 text-sm font-bold rounded-xl transition-all shadow-sm ${filterType === 'request' ? 'bg-orange-500 text-white' : (isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-white text-slate-500 hover:bg-slate-50')}`}>ركاب 🙋‍♂️</button>
          <button onClick={() => setFilterType('delivery')} className={`flex-1 min-w-[110px] py-2.5 text-sm font-bold rounded-xl transition-all shadow-sm ${filterType === 'delivery' ? 'bg-purple-600 text-white' : (isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-white text-slate-500 hover:bg-slate-50')}`}>توصيل طلبات 📦</button>
        </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className={`inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-md ${trip.type === 'offer' ? (isDarkMode ? 'bg-emerald-900/40 text-emerald-400' : 'bg-emerald-100 text-emerald-800') : trip.type === 'delivery' ? (isDarkMode ? 'bg-purple-900/40 text-purple-400' : 'bg-purple-100 text-purple-800') : (isDarkMode ? 'bg-orange-900/40 text-orange-400' : 'bg-orange-100 text-orange-800')}`}>
                        {trip.type === 'offer' ? 'سائق (يعرض توصيلة)' : trip.type === 'delivery' ? 'معي طرد / طلبية 📦' : 'راكب (يطلب توصيلة)'}
                      </span>
                    </div>
                  </div>
          ...
          <div className={`${bgModal} rounded-[2rem] w-full max-w-lg shadow-2xl border overflow-hidden ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
            ...
            <div className="p-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
              <form onSubmit={handleAddTrip} className="space-y-6">
                ...
                {/* كروت اختيار النوع (راعينا إضافة كارت الدليفري) */}
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    >
                      ...
                      <span className={`font-extrabold text-[11px] sm:text-xs ${newTrip.type === 'request' ? (isDarkMode ? 'text-indigo-400' : 'text-indigo-700') : (isDarkMode ? 'text-slate-400' : 'text-slate-500')}`}>راكب</span>
                    </div>
                    ...
                    >
                      ...
                      <span className={`font-extrabold text-[11px] sm:text-xs text-center ${newTrip.type === 'offer' ? (isDarkMode ? 'text-emerald-400' : 'text-emerald-700') : (isDarkMode ? 'text-slate-400' : 'text-slate-500')}`}>سائق سيارة</span>
                    </div>
                    ...
                    >
                      <Package size={24} className={newTrip.type === 'delivery' ? (isDarkMode ? 'text-purple-400' : 'text-purple-500') : (isDarkMode ? 'text-slate-400' : 'text-slate-400')}/>
                      <span className={`font-extrabold text-[11px] sm:text-xs text-center ${newTrip.type === 'delivery' ? (isDarkMode ? 'text-purple-400' : 'text-purple-700') : (isDarkMode ? 'text-slate-400' : 'text-slate-500')}`}>توصيل طلب</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <textarea rows="3" value={newTrip.notes} onChange={(e) => setNewTrip({...newTrip, notes: e.target.value})} placeholder={newTrip.type === 'delivery' ? "تفاصيل الطرد (نوعه، قابل للكسر، رقم المستلم...)" : "تفاصيل إضافية (أماكن الوقوف بالتحديد، حجم الحقائب، الخ...)"} className={`w-full border p-4 rounded-2xl resize-none font-medium outline-none focus:ring-2 focus:ring-indigo-500 transition-colors leading-relaxed ${bgInput}`}></textarea>
                </div>
