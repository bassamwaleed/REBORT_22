...
// إعدادات فايربيز - لم يتم المساس بها
...
// داتا وهمية للبريزنتيشن - لم يتم المساس بها
const DUMMY_TRIPS = [
    ...
    return (
      <div dir="rtl" className={`min-h-screen flex items-center justify-center p-4 transition-colors ${bgMain} bg-[url('data:image/svg+xml;base64,')]`}>
        <div className={`${bgCard} p-8 sm:p-10 rounded-3xl shadow-2xl max-w-md w-full border-t-8 border-indigo-600 relative overflow-hidden`}>
          ...
          <p className={`${textMuted} mb-8 text-center text-sm font-medium`}>{isLoginMode ? 'مرحباً بعودتك! سجل دخولك للمتابعة' : 'انضم إلينا وابدأ رحلتك التوفيرية'}</p>
          ...
          <div className="mt-6 text-center space-y-5">
            <button onClick={() => setIsLoginMode(!isLoginMode)} className="text-indigo-600 dark:text-indigo-400 font-bold text-sm hover:underline block w-full transition-all">
            ...
            <div className="flex items-center justify-center gap-3 text-slate-400 text-sm">
              <span className="w-12 h-px bg-slate-200 dark:bg-slate-700"></span> أو <span className="w-12 h-px bg-slate-200 dark:bg-slate-700"></span>
            </div>
    ...
    <div dir="rtl" className={`min-h-screen flex flex-col relative transition-colors duration-300 ${bgMain}`}>
      ...
      {systemNotice && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex justify-center items-center p-4">
          <div className={`${bgModal} rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl border border-indigo-500/30 transform scale-105 transition-transform`}>
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-8 leading-relaxed font-medium">{systemNotice.body}</p>
      ...
      {showAuthPrompt && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[70] flex justify-center items-center p-4">
          <div className={`${bgModal} rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl border border-slate-700`}>
            ...
            <p className={`${textMuted} mb-8 text-sm leading-relaxed`}>عشان تقدر تستخدم الميزة دي وتتفاعل مع الرحلات، لازم تسجل حساب معانا في ثواني.</p>
          ...
          <div className={`${bgModal} w-full sm:w-[400px] h-full shadow-2xl flex flex-col animate-fade-in-right`}>
            ...
            <div className="p-6 flex-1 overflow-y-auto space-y-6">
              ...
              <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-700/50 border-slate-600' : 'bg-slate-50 border-slate-200'}`}>
                ...
                <p className={`${textMuted} text-xs mb-4 mt-2 leading-relaxed`}>احصل على العلامة لزيادة الثقة والمصداقية لرحلاتك.</p>
              ...
              <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-700/50 border-slate-600' : 'bg-slate-50 border-slate-200'}`}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="p-2 bg-slate-200 dark:bg-slate-800 rounded-xl"><Info className="text-slate-600 dark:text-slate-400" size={20}/></div>
            ...
            <div className={`p-6 border-t ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
              ...
              <button onClick={handleLogout} className="w-full py-3.5 bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400 rounded-xl font-bold flex justify-center items-center gap-2 hover:bg-rose-100 dark:hover:bg-rose-500/20 transition-colors">
            ...
            <div className={`flex-1 overflow-y-auto p-5 space-y-4 ${isDarkMode ? 'bg-slate-900/50' : 'bg-slate-50'}`}>
              {myOwnTrips.length === 0 ? (
                <div className="text-center text-slate-500 mt-20 flex flex-col items-center">
                  <div className="w-24 h-24 bg-slate-200 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4"><Car size={40} className="text-slate-400"/></div>
                ...
                myOwnTrips.map(trip => (
                  <div key={trip.id} className={`p-5 rounded-2xl shadow-sm border relative flex flex-col ${bgCard}`}>
                    <div className="flex justify-between items-start mb-4">
                      <span className={`text-xs font-bold px-3 py-1.5 rounded-lg ${trip.type === 'offer' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400'}`}>
                        {trip.type === 'offer' ? 'أنا سائق' : 'أنا راكب'}
                      </span>
                      <span className={`text-xs font-bold px-3 py-1.5 rounded-lg ${trip.status === 'completed' ? 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300' : trip.status === 'in_progress' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400'}`}>
                        {trip.status === 'completed' ? 'مكتملة ✅' : trip.status === 'in_progress' ? 'في الطريق 🚗' : 'متاحة الآن'}
                      </span>
                    </div>
                    
                    <div className="relative mb-5">
                      <div className="absolute right-[7px] top-2 bottom-2 w-0.5 bg-slate-200 dark:bg-slate-700"></div>
                      <div className="flex items-center gap-4 mb-4 relative z-10">
                        <div className="w-4 h-4 rounded-full bg-indigo-500 border-2 border-white dark:border-slate-800 mt-0.5 shrink-0"></div>
                        <p className="font-bold text-sm text-slate-800 dark:text-slate-100">{trip.from}</p>
                      </div>
                      <div className="flex items-center gap-4 relative z-10">
                        <div className="w-4 h-4 rounded-full bg-rose-500 border-2 border-white dark:border-slate-800 mt-0.5 shrink-0"></div>
                        <p className="font-bold text-sm text-slate-800 dark:text-slate-100">{trip.to}</p>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-2 mt-auto border-t pt-4 border-slate-100 dark:border-slate-700">
                      {(trip.status === 'open' || !trip.status) && (
                        <button onClick={() => handleUpdateTripStatus(trip.id, 'in_progress')} className="flex-1 text-xs py-2.5 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400 font-bold rounded-xl hover:bg-indigo-100 dark:hover:bg-indigo-900/40 flex items-center justify-center gap-2 transition-colors"><Play size={16}/> بدء الرحلة</button>
                      )}
                      {trip.status === 'in_progress' && (
                        <button onClick={() => handleUpdateTripStatus(trip.id, 'completed')} className="flex-1 text-xs py-2.5 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400 font-bold rounded-xl hover:bg-emerald-100 dark:hover:bg-emerald-900/40 flex items-center justify-center gap-2 transition-colors"><CheckSquare size={16}/> إنهاء الرحلة</button>
                      )}
                      <button onClick={() => handleDeleteTrip(trip.id)} className="text-xs py-2.5 px-4 bg-rose-50 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400 font-bold rounded-xl hover:bg-rose-100 dark:hover:bg-rose-900/40 flex items-center justify-center gap-2 transition-colors"><Trash2 size={16}/></button>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            {!isGuest && (
              <button onClick={() => setShowInbox(true)} className={`relative p-2.5 rounded-xl transition-all ${isDarkMode ? 'hover:bg-slate-800 text-slate-400 hover:text-slate-200' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-800'}`}>
                <Bell size={22} />
                {myInbox.length > 0 && <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-white dark:border-slate-900"></span>}
              </button>
            )}
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-6 w-full pb-28 md:pb-6">
        ...
        <div className="bg-gradient-to-br from-indigo-600 via-blue-700 to-indigo-900 rounded-[2rem] p-6 sm:p-10 mb-8 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl">
            ...
            <div className="bg-white/10 backdrop-blur-md p-2 rounded-2xl flex flex-col sm:flex-row gap-2 shadow-inner">
              <div className="flex-1 flex items-center bg-white dark:bg-slate-800 rounded-xl px-4 py-3.5 transition-colors">
                <MapPin className="text-indigo-500 ml-3" size={20} />
                <input type="text" placeholder="من (مثال: رمسيس)" value={searchFrom} onChange={(e) => setSearchFrom(e.target.value)} className="bg-transparent border-none w-full text-slate-800 dark:text-white outline-none font-bold placeholder-slate-400" />
              </div>
              <div className="flex-1 flex items-center bg-white dark:bg-slate-800 rounded-xl px-4 py-3.5 transition-colors">
                <Navigation className="text-rose-500 ml-3" size={20} />
                <input type="text" placeholder="إلى (مثال: المنصورة)" value={searchTo} onChange={(e) => setSearchTo(e.target.value)} className="bg-transparent border-none w-full text-slate-800 dark:text-white outline-none font-bold placeholder-slate-400" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className={`flex gap-2 p-1.5 mb-8 max-w-lg mx-auto rounded-2xl shadow-inner ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
          <button 
            ...
            className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-all shadow-sm ${filterType === 'all' ? 'bg-slate-800 text-white dark:bg-slate-200 dark:text-slate-900' : 'bg-white text-slate-500 hover:bg-slate-50 dark:bg-slate-800 dark:text-slate-400'}`}>
            الكل
          </button>
          <button 
            onClick={() => setFilterType('offer')} 
            className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-all shadow-sm ${filterType === 'offer' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50 dark:bg-slate-800 dark:text-slate-400'}`}>
            أصحاب سيارات 🚗
          </button>
          <button 
            onClick={() => setFilterType('request')} 
            className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-all shadow-sm ${filterType === 'request' ? 'bg-orange-500 text-white' : 'bg-white text-slate-500 hover:bg-slate-50 dark:bg-slate-800 dark:text-slate-400'}`}>
            ركاب 🙋‍♂️
          </button>
        </div>
        ...
        {filteredTrips.length === 0 ? (
          <div className={`text-center py-24 rounded-[2rem] border-2 border-dashed flex flex-col items-center justify-center ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-200 bg-slate-50/50'}`}>
             <div className="w-20 h-20 bg-slate-200 dark:bg-slate-700 rounded-full flex items-center justify-center mb-4"><Car size={32} className="text-slate-400"/></div>
            ...
            {filteredTrips.map(trip => {
              ...
              <div key={trip.id} className={`rounded-[24px] p-6 shadow-sm hover:shadow-lg hover:-translate-y-1 border relative flex flex-col transition-all duration-300 ${bgCard} ${isCompleted ? 'opacity-75 grayscale-[20%]' : ''}`}>
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center border border-slate-200 dark:border-slate-600 shadow-sm shrink-0">
                      <User size={24} className="text-slate-400" />
                    </div>
                    <div>
                      <h3 className="font-bold text-sm flex items-center gap-1 text-slate-800 dark:text-white">
                        {trip.userName} {trip.verified && <ShieldCheck size={16} className="text-blue-500" />}
                      </h3>
                      {/* البادج: لون مميز بناءً على نوع الرحلة */}
                      <span className={`inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-md ${trip.type === 'offer' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-400' : 'bg-orange-100 text-orange-800 dark:bg-orange-900/40 dark:text-orange-400'}`}>
                  ...
                  {/* حالة الرحلة */}
                  <div className={`text-[10px] font-bold px-2.5 py-1 rounded-lg border ${isCompleted ? 'bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:border-slate-600' : isInProgress ? 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800 animate-pulse' : 'bg-indigo-50 text-indigo-600 border-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-800'}`}>
                ...
                <div className="relative mb-6 flex-1">
                  {/* الخط الواصل: تم ضبط موقعه ليكون في منتصف الدوائر تماماً */}
                  <div className="absolute right-[7px] top-2 bottom-2 w-0.5 bg-slate-200 dark:bg-slate-600"></div>
                  
                  <div className="flex items-start gap-4 mb-4 relative z-10">
                    <div className="w-4 h-4 rounded-full bg-indigo-500 border-[3px] border-white dark:border-slate-800 mt-0.5 shrink-0 shadow-sm"></div>
                    <p className="font-bold text-sm text-slate-800 dark:text-slate-100 leading-snug">{trip.from}</p>
                  </div>
                  
                  <div className="flex items-start gap-4 relative z-10">
                    <div className="w-4 h-4 rounded-full bg-rose-500 border-[3px] border-white dark:border-slate-800 mt-0.5 shrink-0 shadow-sm"></div>
                    <p className="font-bold text-sm text-slate-800 dark:text-slate-100 leading-snug">{trip.to}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-3 mb-5">
                  <div className={`flex-1 flex flex-col justify-center items-center py-2.5 rounded-xl border ${isDarkMode ? 'bg-slate-700/30 border-slate-600' : 'bg-slate-50 border-slate-100'}`}>
                     ...
                     <span className="text-sm font-extrabold flex items-center gap-1.5 text-slate-700 dark:text-slate-200"><User size={15} className="text-indigo-400"/> {trip.seats}</span>
                  </div>
                  {trip.cost && (
                    <div className={`flex-1 flex flex-col justify-center items-center py-2.5 rounded-xl border ${isDarkMode ? 'bg-slate-700/30 border-slate-600' : 'bg-slate-50 border-slate-100'}`}>
                      <span className="text-[10px] text-slate-400 mb-1">المساهمة</span>
                      <span className="text-sm font-extrabold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5"><Wallet size={15}/> {trip.cost} ج</span>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-auto">
                  {isOwner ? (
                    ...
                    isCompleted ? (
                      ...
                      <div className={`w-full py-3.5 rounded-xl font-bold text-center text-sm bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-400`}>
                        الرحلة جارية الآن 🚗
                      </div>
                    )
                  ) : (
          ...
          <div className={`${bgModal} w-full h-[80vh] sm:h-[600px] sm:max-w-md rounded-t-[2rem] sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden`}>
            ...
            <div className={`flex-1 overflow-y-auto p-4 ${isDarkMode ? 'bg-slate-900/50' : 'bg-slate-50'}`}>
              {myInbox.length === 0 ? (
                ...
                myInbox.map(chat => (
                  <div key={chat.chatId} onClick={() => { setShowInbox(false); setActiveChat(chat); }} className={`p-4 rounded-2xl shadow-sm mb-3 cursor-pointer border hover:border-indigo-300 transition-all ${bgCard}`}>
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                        <h4 className="font-bold text-sm">{chat.otherPersonName}</h4>
                        <p className="text-xs text-slate-500 line-clamp-1 mt-1">{chat.lastMessage}</p>
                       </div>
                    </div>
                  </div>
                ))
              ...
              {messages.map(msg => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-3 rounded-2xl text-sm shadow-sm ${isMe ? 'bg-indigo-600 text-white rounded-tl-sm' : (isDarkMode ? 'bg-slate-800 text-white rounded-tr-sm border border-slate-700' : 'bg-white border border-slate-200 text-slate-800 rounded-tr-sm')}`}>
                      {msg.text}
                    </div>
                  </div>
                )
          ...
          <div className={`${bgModal} rounded-[2rem] w-full max-w-lg shadow-2xl border overflow-hidden ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
            ...
            <div className="p-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
              <form onSubmit={handleAddTrip} className="space-y-6">
                ...
                {/* اختيار النوع */}
                <div>
                  <label className="block text-sm font-bold mb-3 text-slate-700 dark:text-slate-300">أنت مسافر بصفتك؟</label>
                  <div className="grid grid-cols-2 gap-4">
                    <div 
                      ...
                      className={`cursor-pointer p-4 rounded-2xl border-2 flex flex-col items-center justify-center gap-2 transition-all ${
                        newTrip.type === 'request' 
                          ? 'border-indigo-600 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 shadow-sm' 
                          : (isDarkMode ? 'border-slate-700 bg-slate-800 text-slate-400 hover:bg-slate-700' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50')
                      }`}
                    >
                      <User size={28} className={newTrip.type === 'request' ? 'text-indigo-600 dark:text-indigo-400' : ''}/>
                      <span className="font-extrabold text-sm">راكب</span>
                    </div>
                    ...
                    <div 
                      ...
                      className={`cursor-pointer p-4 rounded-2xl border-2 flex flex-col items-center justify-center gap-2 transition-all ${
                        newTrip.type === 'offer' 
                          ? 'border-emerald-500 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 shadow-sm' 
                          : (isDarkMode ? 'border-slate-700 bg-slate-800 text-slate-400 hover:bg-slate-700' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50')
                      }`}
                    >
                      <Car size={28} className={newTrip.type === 'offer' ? 'text-emerald-500 dark:text-emerald-400' : ''}/>
                      <span className="font-extrabold text-sm">سائق سيارة</span>
                    </div>
                  </div>
                </div>
