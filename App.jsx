...
const DUMMY_TRIPS = [
  ...
  { id: 'dummy_3', type: 'delivery', from: 'الزقازيق', to: 'العاشر من رمضان', date: '2026-08-24', time: '02:00 م', seats: 1, cost: '50', notes: 'توصيل أوراق هامة وطرود سريعة', userId: 'd3', userName: 'محمود حسن', userPhone: '', verified: true, isDummy: true, status: 'in_progress', rating: 4.0, totalRatings: 8, createdAt: { toMillis: () => Date.now() - 3000000 } }
];
...
export default function App() {
  ...
  // جلب إعدادات التطبيق (النصوص والبانرات)
  useEffect(() => {
  ...
  // مراقبة المستخدم وفحص الأدمن بالإيميل الشخصي
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        if (currentUser.isAnonymous) {
          ...
          setIsGuest(false);
          const isUserAdmin = currentUser.email && currentUser.email.toLowerCase() === "bassamwaleed2000@gmail.com";
          ...
          if (userDoc.exists()) {
            ...
            const initialData = { name: currentUser.displayName, photoURL: currentUser.photoURL, isVerified: false, rating: 0, totalRatings: 0, email: currentUser.email };
  ...
  // بوت توليد الرحلات التلقائية (للأدمن)
  const adminGenerateBotTrip = async () => {
    const cities = ["القاهرة", "الإسكندرية", "المنصورة", "طنطا", "الزقازيق", "أسيوط", "الأسماعيلية"];
    const names = ["كريم إبراهيم", "محمود سعيد", "ياسمين عبد الله", "أحمد الشريف", "منى عادل"];
    ...
    try {
      const docRef = await addDoc(collection(db, APP_COLLECTION_NAME), {
        type: randomType,
        from: randomFrom,
        to: randomTo,
        date: '2026-08-27',
        ...
        seats: 1,
        cost: '',
        notes: 'رحلة عشوائية تم توليدها بواسطة النظام',
        ...
        rating: 4.8,
        totalRatings: 5,
        status: 'open', // تبدأ متاحة
        isBot: true,
        createdAt: serverTimestamp()
      });

      // بعد 5 دقايق تحول في الطريق
      setTimeout(async () => {
        await updateDoc(doc(db, APP_COLLECTION_NAME, docRef.id), { status: 'in_progress' });
      }, 5 * 60 * 1000);

      // بعد 30 دقيقة تحول مكتملة
      setTimeout(async () => {
        await updateDoc(doc(db, APP_COLLECTION_NAME, docRef.id), { status: 'completed' });
      }, 30 * 60 * 1000);

      triggerToast('تم توليد رحلة عشوائية بنجاح! 🤖');
    } catch(err) {
      alert('خطأ في توليد الرحلة.');
    }
  };
  ...
  const handleAvatarUpload = async (e) => {
    ...
    try {
      ...
      alert('حدث خطأ. تأكد من تفعيل Storage في إعدادات فايربيز الخاص بك.');
    } finally {
      setIsUploading(false);
    }
  };

  // دوال الأدمن لصور البانر والنصوص
  ...
  const handleAddTrip = async (e) => {
    ...
    try {
      ...
      triggerToast('تم نشر طلبك بنجاح!');
    ...
    return (
      <div className="flex items-center gap-1">
        <div className="flex text-amber-400">
          {[...Array(5)].map((_, i) => (
            <Star key={i} size={12} fill={i < fullStars ? "currentColor" : (i === fullStars && hasHalfStar ? "url(#half)" : "none")} className={i < fullStars || (i === fullStars && hasHalfStar) ? "text-amber-400" : "text-slate-300 dark:text-slate-600"}/>
          ))}
        </div>
        <span className="text-[11px] font-bold text-slate-500">({rating.toFixed(1)})</span>
      </div>
    );
  };
            ...
            {!isLoginMode && (
              <>
                <div className="relative group">
                  <User size={20} className={`absolute right-4 top-4 transition-colors ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`} />
                  <input type="text" required placeholder="الاسم الكامل" value={authForm.name} onChange={e => setAuthForm({...authForm, name: e.target.value})} className={`w-full border rounded-2xl py-3.5 px-12 outline-none focus:ring-2 focus:ring-indigo-500 font-medium ${bgInput}`} />
                </div>
                <div className="relative group">
                  <Phone size={20} className={`absolute right-4 top-4 transition-colors ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`} />
                  <input type="tel" required placeholder="رقم الموبايل" value={authForm.phone} onChange={e => setAuthForm({...authForm, phone: e.target.value})} className={`w-full border rounded-2xl py-3.5 px-12 outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-left ${bgInput}`} dir="ltr" />
                </div>
              </>
            )}
            <div className="relative group">
              <Mail size={20} className={`absolute right-4 top-4 transition-colors ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`} />
              <input type="email" required placeholder="البريد الإلكتروني" value={authForm.email} onChange={e => setAuthForm({...authForm, email: e.target.value})} className={`w-full border rounded-2xl py-3.5 px-12 outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-left ${bgInput}`} dir="ltr" />
            </div>
            <div className="relative group">
              <Lock size={20} className={`absolute right-4 top-4 transition-colors ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`} />
              <input type="password" required placeholder="كلمة المرور" value={authForm.password} onChange={e => setAuthForm({...authForm, password: e.target.value})} className={`w-full border rounded-2xl py-3.5 px-12 outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-left ${bgInput}`} dir="ltr" />
            </div>
            <button type="submit" disabled={authLoading} className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white py-4 rounded-2xl font-bold text-lg hover:from-indigo-700 hover:to-blue-700 shadow-lg flex justify-center items-center gap-2 transform active:scale-[0.98]">
              {authLoading ? <Loader2 className="animate-spin" /> : (isLoginMode ? 'تسجيل الدخول' : 'إنشاء حساب جديد')}
            </button>
          </form>
          <div className="mt-6 text-center space-y-5">
            <button onClick={() => setIsLoginMode(!isLoginMode)} className={`font-bold text-sm hover:underline block w-full ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}>
            ...
            <button onClick={handleGuestLogin} disabled={authLoading} className={`w-full py-3.5 rounded-2xl font-bold border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-200 hover:bg-slate-700' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50'}`}>
              تصفح التطبيق كزائر 👀
            </button>
          </div>
        </div>
  ...
  return (
    <div dir="rtl" className={`min-h-screen flex flex-col relative transition-colors duration-300 ${bgMain}`}>
      ...
      {/* لوحة تحكم الإدارة الشاملة (للنصوص والبانرات والمستخدمين والبوت) */}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[80] flex justify-center items-center p-4">
          <div className={`${bgModal} rounded-3xl w-full max-w-2xl h-[85vh] shadow-2xl border flex flex-col ${isDarkMode ? 'border-amber-500/30' : 'border-amber-400'}`}>
            ...
            <div className={`flex-1 overflow-y-auto p-6 space-y-8 ${isDarkMode ? 'bg-slate-900' : 'bg-slate-50'}`}>
              ...
              {/* قسم توليد الرحلات العشوائية (البوت) */}
              <div className={`p-5 rounded-2xl border ${bgCard}`}>
                <h3 className="font-bold text-lg mb-2 flex items-center gap-2"><Sparkles size={18} className="text-amber-500"/> توليد نشاط وهمي (بوت)</h3>
                <p className="text-xs text-slate-400 mb-4">انقر لتوليد رحلة عشوائية تتحرك بمرور الوقت (في الطريق خلال 5 دقائق ومكتملة خلال 30 دقيقة).</p>
                <button onClick={adminGenerateBotTrip} className="w-full py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold rounded-xl shadow-md hover:opacity-90">
                  🤖 توليد رحلة أو طلب دليفري عشوائي الآن
                </button>
              </div>
              ...
              {/* قسم الإعلانات النصية */}
              <div className={`p-5 rounded-2xl border ${bgCard}`}>
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2"><Megaphone size={18} className="text-indigo-500"/> إدارة شريط الأخبار المتحرك</h3>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex justify-end">
          <div className={`${bgModal} w-full sm:w-[400px] h-full shadow-2xl flex flex-col animate-fade-in-right`}>
            ...
            <div className="p-6 flex-1 overflow-y-auto space-y-6">
              ...
              {isAdmin && (
                <button onClick={() => {setShowSettings(false); setShowAdminPanel(true);}} className="w-full p-4 rounded-2xl flex justify-between items-center bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg transform active:scale-95 transition-all">
      ...
      <header className={`sticky top-0 z-40 transition-all duration-300 border-b backdrop-blur-md ${isDarkMode ? 'bg-slate-900/90 border-slate-800 shadow-md' : 'bg-white/90 border-slate-200 shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-4 h-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            {!isGuest && (
              <button onClick={() => setShowMyTrips(true)} className={`p-2.5 rounded-xl transition-all ${isDarkMode ? 'hover:bg-slate-800 text-slate-400 hover:text-slate-200' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-800'}`}>
                <History size={22} />
              </button>
            )}
            {!isGuest && (
            ...
            <button onClick={() => setShowSettings(true)} className={`relative p-2.5 rounded-xl transition-all ${isDarkMode ? 'hover:bg-slate-800 text-slate-400 hover:text-slate-200' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-800'}`}>
              {userData?.photoURL ? <img src={userData.photoURL} className="w-6 h-6 rounded-full object-cover border border-slate-300" /> : <Settings size={22} />}
            </button>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-6 w-full pb-28 md:pb-6">
        ...
        {/* البانر التفاعلي بالصور المتحركة */}
        <div className="relative rounded-[2.5rem] p-7 sm:p-10 mb-8 shadow-xl overflow-hidden min-h-[280px] flex items-center group">
          {bannerImages.length > 0 ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-600 via-blue-700 to-indigo-900 z-0">
          ...
          <div className="relative z-10 max-w-2xl text-white">
            ...
            <h1 className="text-3xl sm:text-4xl font-black mb-3 leading-tight drop-shadow-lg">شارك رحلتك، واجعل كل مشوار أسهل</h1>
            ...
            <button onClick={() => requireAuth(() => setShowAddModal(true))} className="bg-white text-indigo-700 font-extrabold px-6 py-3.5 rounded-2xl shadow-lg hover:bg-indigo-50 transition-all flex items-center gap-2 transform active:scale-95">
        ...
        <div className={`flex gap-2 p-1.5 mb-8 max-w-xl mx-auto rounded-2xl shadow-inner ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
          ...
          <button onClick={() => setFilterType('offer')} className={`flex-1 py-2.5 text-xs sm:text-sm font-bold rounded-xl transition-all shadow-sm ${filterType === 'offer' ? 'bg-emerald-600 text-white' : (isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-white text-slate-500 hover:bg-slate-50')}`}>سواقين 🚗</button>
        ...
        {filteredTrips.length === 0 ? (
          <div className={`text-center py-24 rounded-[2rem] border-2 border-dashed flex flex-col items-center justify-center ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-200 bg-slate-50/50'}`}>
            ...
            <h3 className="text-xl font-bold mb-2">لا توجد رحلات أو طلبات مطابقة</h3>
            <p className="text-slate-500 text-sm">جرب تغيير كلمات البحث أو انشر رحلتك أو طلبك الآن!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTrips.map(trip => {
              ...
              const isDelivery = trip.type === 'delivery';
              ...
              <div key={trip.id} className={`rounded-[24px] p-6 shadow-sm hover:shadow-lg hover:-translate-y-1 border relative flex flex-col transition-all duration-300 ${bgCard} ${isCompleted ? 'opacity-75 grayscale-[20%]' : ''}`}>
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="flex items-center gap-3">
                    {trip.userPhoto ? (
                      <img src={trip.userPhoto} className="w-12 h-12 rounded-full object-cover border border-slate-200 shadow-sm shrink-0" />
                    ) : (
                    ...
                    <div>
                      ...
                      <span className={`inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-md ${isDelivery ? (isDarkMode ? 'bg-purple-900/40 text-purple-400' : 'bg-purple-100 text-purple-800') : trip.type === 'offer' ? (isDarkMode ? 'bg-emerald-900/40 text-emerald-400' : 'bg-emerald-100 text-emerald-800') : (isDarkMode ? 'bg-orange-900/40 text-orange-400' : 'bg-orange-100 text-orange-800')}`}>
                        {isDelivery ? '📦 طلب توصيل دليفري' : trip.type === 'offer' ? 'سائق (يعرض توصيلة)' : 'راكب (يطلب توصيلة)'}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex gap-3 mb-5">
                  <div className={`flex-1 flex flex-col justify-center items-center py-2.5 rounded-xl border ${isDarkMode ? 'bg-slate-700/30 border-slate-600' : 'bg-slate-50 border-slate-100'}`}>
                     <span className="text-[10px] text-slate-400 mb-1">{isDelivery ? 'نوع الطلب' : 'العدد المطلوب'}</span>
                     <span className={`text-sm font-extrabold flex items-center gap-1.5 ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>
                       {isDelivery ? <Package size={15} className="text-purple-400"/> : <User size={15} className="text-indigo-400"/>} 
                       {isDelivery ? 'طرد / منتج' : trip.seats}
                     </span>
                  </div>
                  {trip.cost && (
                    <div className={`flex-1 flex flex-col justify-center items-center py-2.5 rounded-xl border ${isDarkMode ? 'bg-slate-700/30 border-slate-600' : 'bg-slate-50 border-slate-100'}`}>
                ...
                <div className="mt-auto">
                  {isOwner ? (
                    <div className={`w-full py-3.5 rounded-xl font-bold text-center text-sm border border-dashed ${isDarkMode ? 'bg-slate-700/50 text-slate-400 border-slate-600' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>
                      {isCompleted ? 'طلبك (مكتمل)' : isInProgress ? 'طلبك (في الطريق)' : 'هذا طلبك'}
                    </div>
                  ) : isClosedForPublic ? (
                    isCompleted ? (
                      isPassenger && !trip.isDummy ? (
                        <div className={`p-3.5 rounded-xl text-center border ${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-200'}`}>
                          <p className="text-xs font-bold mb-2">تقييمك:</p>
      ...
      <button onClick={() => requireAuth(() => setShowAddModal(true))} className="md:hidden fixed bottom-8 left-6 w-14 h-14 bg-indigo-600 text-white rounded-full flex items-center justify-center shadow-xl shadow-indigo-500/40 z-40 transform active:scale-95 transition-transform">
        <Navigation size={24} />
      </button>
                  ...
                  <div key={chat.chatId} onClick={() => { setShowInbox(false); setActiveChat(chat); }} className={`p-4 rounded-2xl shadow-sm mb-3 cursor-pointer border transition-all ${bgCard} ${isDarkMode ? 'hover:border-indigo-500' : 'hover:border-indigo-300'}`}>
                    <div className="flex items-center gap-3">
                       {chat.otherPersonPhoto ? (
                         <img src={chat.otherPersonPhoto} className="w-10 h-10 rounded-full object-cover border border-slate-200" />
                       ) : (
          ...
          <div className={`${bgModal} w-full h-[85vh] sm:h-[600px] sm:max-w-md rounded-t-[2rem] sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden`}>
            <div className="bg-indigo-600 text-white p-4 flex items-center gap-3">
              ...
              {activeChat.otherPersonPhoto ? (
                <img src={activeChat.otherPersonPhoto} className="w-10 h-10 rounded-full object-cover border-2 border-white/20" />
              ) : (
      ...
      {/* شاشة الإضافة (تدعم رحلة أو دليفري) */}
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex justify-center items-center p-4">
          <div className={`${bgModal} rounded-[2rem] w-full max-w-lg shadow-2xl border overflow-hidden ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
            <div className={`flex justify-between items-center p-6 border-b ${isDarkMode ? 'bg-slate-800/80 border-slate-700' : 'bg-slate-50/80 border-slate-100 backdrop-blur-md'}`}>
              <h2 className="text-xl font-extrabold flex items-center gap-2"><Navigation className="text-indigo-500"/> نشر طلب أو رحلة</h2>
            ...
            <div className="p-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
              <form onSubmit={handleAddTrip} className="space-y-6">
                ...
                {/* 3 خيارات لاختيار النوع (راكب / سائق / دليفري) */}
                <div>
                  <label className={`block text-sm font-bold mb-3 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>ماذا تريد أن تفعل اليوم؟</label>
                  <div className="grid grid-cols-3 gap-3">
                    
                    <div 
                      onClick={() => setNewTrip({...newTrip, type: 'request'})} 
                      className={`cursor-pointer p-3 rounded-2xl border-2 flex flex-col items-center justify-center gap-1.5 transition-all ${
                    ...
                    >
                      <User size={22} className={newTrip.type === 'request' ? 'text-indigo-500' : 'text-slate-400'}/>
                      <span className={`font-extrabold text-xs ${newTrip.type === 'request' ? 'text-indigo-600' : 'text-slate-500'}`}>راكب</span>
                    </div>
                    ...
                    >
                      <Car size={22} className={newTrip.type === 'offer' ? 'text-emerald-500' : 'text-slate-400'}/>
                      <span className={`font-extrabold text-xs ${newTrip.type === 'offer' ? 'text-emerald-600' : 'text-slate-500'}`}>سائق سيارة</span>
                    </div>
                    ...
                    <div 
                      ...
                      className={`cursor-pointer p-3 rounded-2xl border-2 flex flex-col items-center justify-center gap-1.5 transition-all ${
                        newTrip.type === 'delivery' 
                          ? (isDarkMode ? 'border-purple-600 bg-purple-900/30 shadow-sm' : 'border-purple-600 bg-purple-50 shadow-sm')
                          : (isDarkMode ? 'border-slate-700 bg-slate-800 hover:bg-slate-700' : 'border-slate-200 bg-white hover:bg-slate-50')
                      }`}
                    >
                      <Package size={22} className={newTrip.type === 'delivery' ? 'text-purple-500' : 'text-slate-400'}/>
                      <span className={`font-extrabold text-xs ${newTrip.type === 'delivery' ? 'text-purple-600' : 'text-slate-500'}`}>توصيل طلب</span>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="relative">
                      ...
                      <input type="text" required value={newTrip.from} onChange={(e) => setNewTrip({...newTrip, from: e.target.value})} placeholder={newTrip.type === 'delivery' ? 'مكان استلام الطلب' : 'نقطة التحرك'} className={`w-full border py-3.5 pr-11 pl-4 rounded-2xl font-bold outline-none focus:ring-2 focus:ring-indigo-500 transition-colors ${bgInput}`} />
                    </div>
                    <div className="relative">
                      <Navigation size={18} className="absolute right-4 top-4 text-slate-400" />
                      <input type="text" required value={newTrip.to} onChange={(e) => setNewTrip({...newTrip, to: e.target.value})} placeholder={newTrip.type === 'delivery' ? 'مكان تسليم الطلب' : 'نقطة الوصول'} className={`w-full border py-3.5 pr-11 pl-4 rounded-2xl font-bold outline-none focus:ring-2 focus:ring-indigo-500 transition-colors ${bgInput}`} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {newTrip.type !== 'delivery' ? (
                      ...
                      <div className="relative">
                        <Package size={18} className="absolute right-4 top-4 text-slate-400" />
                        <input type="text" disabled value="طرد / منتج واحد" className={`w-full border py-3.5 pr-11 pl-4 rounded-2xl font-bold outline-none bg-slate-100 dark:bg-slate-700 text-slate-400`} />
                      </div>
                    )}
                    
                    <div className="relative">
                      <Wallet size={18} className="absolute right-4 top-4 text-slate-400" />
                      <input type="number" min="0" placeholder={newTrip.type === 'delivery' ? 'تكلفة التوصيل (ج)' : 'المساهمة (ج)'} value={newTrip.cost} onChange={(e) => setNewTrip({...newTrip, cost: e.target.value})} className={`w-full border py-3.5 pr-11 pl-4 rounded-2xl font-bold outline-none focus:ring-2 focus:ring-indigo-500 transition-colors ${bgInput}`} />
                    </div>
                  </div>
                  
                  <textarea rows="3" value={newTrip.notes} onChange={(e) => setNewTrip({...newTrip, notes: e.target.value})} placeholder={newTrip.type === 'delivery' ? 'وصف المنتج أو الطلب بالتحديد...' : 'تفاصيل إضافية (أماكن الوقوف، حجم الحقائب...)'} className={`w-full border p-4 rounded-2xl resize-none font-medium outline-none focus:ring-2 focus:ring-indigo-500 transition-colors leading-relaxed ${bgInput}`}></textarea>
                </div>
                ...
                <button type="submit" disabled={isSubmitting} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-extrabold text-lg hover:bg-indigo-700 shadow-lg shadow-indigo-500/30 flex justify-center items-center gap-2 transition-all transform active:scale-[0.98]">
                  {isSubmitting ? <><Loader2 className="animate-spin"/> جاري النشر...</> : 'نشر الطلب الآن'}
                </button>
              </form>
            </div>
          </div>
