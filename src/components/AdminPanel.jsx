    ...
    <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-md z-[500] flex justify-center items-start sm:items-center p-0 sm:p-4 pointer-events-auto">
      <div className={`${bgCard} w-full sm:max-w-3xl h-[100vh] sm:h-[85vh] rounded-none sm:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-fade-in-up border dark:border-slate-700`}>
        ...
        <div className="p-4 border-b flex justify-between items-center bg-indigo-900 text-white shrink-0">
          ...
          <button onClick={onClose} className="p-2 bg-indigo-800 rounded-full hover:bg-indigo-700 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex border-b dark:border-slate-800 shrink-0 bg-slate-50 dark:bg-slate-900">
          <button 
            ...
            className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition-colors ${activeTab === 'verifications' ? 'border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400' : textSecondary}`}
          >
            <ShieldAlert size={18}/> طلبات التوثيق
          ...
          <button 
            ...
            className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition-colors ${activeTab === 'settings' ? 'border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400' : textSecondary}`}
          >
          ...
          {activeTab === 'verifications' && (
            <div className="space-y-4">
              {isLoading ? (
                ...
                <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-[2rem] border dark:border-slate-800">
                  <ShieldCheck size={48} className="mx-auto mb-4 text-slate-300 dark:text-slate-700"/>
                  <h3 className={`font-bold text-lg ${textPrimary}`}>لا يوجد طلبات توثيق حالياً</h3>
                  <p className={`text-sm mt-2 ${textSecondary}`}>كل الحسابات مراجعة وفي السليم!</p>
                </div>
              ) : (
                pendingRequests.map(req => (
                  <div key={req.id} className={`p-4 rounded-[1.5rem] border shadow-sm ${bgCard} flex flex-col gap-4`}>
                    <div className="flex justify-between items-center border-b dark:border-slate-800 pb-3">
                      <div>
                        <h4 className={`font-black text-lg ${textPrimary}`}>{req.name}</h4>
                        <p className={`text-xs ${textSecondary} mt-1`}>{req.phone || req.email}</p>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <p className="text-xs font-bold mb-2">صورة الوجه الأمامي:</p>
                        <img src={req.idFront} alt="الوجه الأمامي" className="w-full h-40 object-cover rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800" />
                      </div>
                      <div>
                        <p className="text-xs font-bold mb-2">صورة الوجه الخلفي:</p>
                        <img src={req.idBack} alt="الوجه الخلفي" className="w-full h-40 object-cover rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800" />
                      </div>
                    </div>

                    <div className="flex gap-2 mt-2">
                      <button onClick={() => handleVerificationAction(req.id, 'approve')} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors">
                        <Check size={18}/> قبول التوثيق
                      </button>
                      <button onClick={() => handleVerificationAction(req.id, 'reject')} className="flex-1 bg-rose-500 hover:bg-rose-600 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors">
                        <XCircle size={18}/> رفض
                      </button>
                    </div>
                  </div>
                ...
                <div className="w-full h-40 rounded-2xl border-2 border-dashed border-indigo-300 dark:border-slate-700 flex items-center justify-center overflow-hidden bg-slate-50 dark:bg-slate-800 relative group mb-4">
                  {appSettings.banner ? (
                    ...
                    <div className="text-center text-slate-400">
                      ...
                      <span className="text-sm font-bold">لا يوجد بانر حالياً</span>
                    </div>
                  )}
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <label className="bg-white text-slate-900 px-4 py-2 rounded-xl text-sm font-bold cursor-pointer hover:scale-105 transition-transform">
                      تغيير البانر
