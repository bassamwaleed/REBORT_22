  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, `inbox_${user.uid}`, baseChatData.chatId), (docSnap) => {
      if(docSnap.exists()) {
        ...
        setLiveChatInfo(prev => ({...prev, ...data}));
        if (data.requestStatus === 'arrived' && data.requestSenderId === user.uid) {
      ...
      <div className={`bg-white dark:bg-slate-900 w-full h-[90vh] sm:h-[650px] sm:max-w-md md:max-w-2xl rounded-t-[2rem] sm:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-fade-in-up relative border dark:border-slate-700`}>
        ...
        {!liveChatInfo?.isGroup && liveChatInfo?.tripInfo !== 'system' && (
          <div className={`p-3 border-b text-center shadow-sm z-10 space-y-2 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
            ...
            {reqStatus === 'none' && !isRequestSender && !isTripOwner && ( 
              <button onClick={() => onActionClick('request')} className="w-full bg-indigo-600 text-white font-bold py-3 rounded-xl text-sm shadow-md hover:bg-indigo-700 active:scale-95 transition-transform">إرسال طلب تأكيد الرحلة 🙋‍♂️</button> 
            )}
            ...
            {reqStatus === 'completed' && <div className="text-emerald-500 font-bold text-xs p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800">الرحلة مكتملة بنجاح ✅</div>}
            ...
            {reqStatus === 'rejected' && <div className="text-rose-500 font-bold text-xs p-3 bg-rose-50 dark:bg-rose-900/20 rounded-xl border border-rose-200 dark:border-rose-800">تم رفض الطلب ❌</div>}
          </div>
        )}
        ...
        <div className={`p-3 sm:p-4 border-t ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            <form onSubmit={handleSendMessage} className="flex gap-2 items-center">
              <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="اكتب رسالتك..." className={`flex-1 rounded-2xl px-4 py-4 text-sm outline-none focus:ring-2 focus:ring-indigo-500 ${bgInput}`} />
              <button type="submit" disabled={!newMessage.trim()} className="bg-indigo-600 text-white w-14 h-14 flex items-center justify-center rounded-2xl hover:bg-indigo-700 shadow-md transition-transform active:scale-95 disabled:opacity-50">
