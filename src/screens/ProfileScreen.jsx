...
import { User, ShieldCheck, Star, Edit2, ShieldAlert, Gift, ChevronLeft, Moon, LogOut, Crown } from 'lucide-react';

const ProfileScreen = ({
  user, userData, isGuest, isAdmin, isDarkMode, toggleTheme, handleLogout, 
  handleEditName, setShowVerifyModal, setShowRewardsModal, setShowAdminPanel, forceSignUpScreen
}) => {
      ...
      <div className="space-y-4">
        ...
        <div className={`p-6 rounded-[2rem] border shadow-sm flex flex-col items-center ${bgCard}`}>
          ...
          <h3 className={`font-black text-xl flex items-center gap-1.5 ${textPrimary}`}>
            ...
            {/* زر تعديل الاسم موجود هنا 👇 */}
            {handleEditName && <button onClick={handleEditName} className="text-slate-400 hover:text-indigo-600 transition-colors ml-1 p-1"><Edit2 size={16}/></button>}
