...
const HomeScreen = ({
  ...
  const visibleTrips = (realTrips || []).filter(t => {
    ...
    if (homeCategory === 'travel') {
       return t.category === 'travel' && 
              t.type === filterType && 
              (!filterFrom || t.from === filterFrom) && 
              (!filterTo || t.to === filterTo);
    }
    if (homeCategory === 'parcel') {
       return (t.category === 'parcel' || t.type === 'delivery') && 
              (!filterFrom || t.from === filterFrom) && 
              (!filterTo || t.to === filterTo);
    }
    if (homeCategory === 'weekend') {
       return t.category === 'weekend' && 
              (!filterFrom || t.from === filterFrom) && 
              (!filterTo || t.to === filterTo);
    }
    return false;
  ...
  return (
    <main className="flex-1 w-full max-w-2xl mx-auto px-4 py-6 relative z-10 pb-[100px]">
            ...
            <div className="mb-6 space-y-4">
              ...
              <div className={`flex gap-2 p-3 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                ...
                <select value={filterTo} onChange={(e) => setFilterTo(e.target.value)} className={`flex-1 p-2.5 rounded-xl border text-xs font-bold outline-none ${bgInput}`}>
                  <option value="">إلى الوجهة</option>
                  {EGYPT_CITIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                {(filterFrom || filterTo) && (
                  <button onClick={() => {setFilterFrom(''); setFilterTo('');}} className="p-2.5 bg-rose-100 text-rose-600 rounded-xl hover:bg-rose-200 transition-colors">
                    <X size={16}/>
                  </button>
                )}
              </div>
             ...
             {homeCategory === 'stations' ? (
               stations.length === 0 ? (
                 <div className={`text-center py-20 rounded-[2rem] border-2 border-dashed ${isDarkMode ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-slate-50'}`}>
                   ...
                   <h3 className="font-bold text-slate-500">جاري تحميل المواقف...</h3>
                 </div>
               ) : (
                    ...
                    <div key={station.id} onClick={() => setSelectedStation(station)} className={`p-4 rounded-[1.5rem] border shadow-sm hover:shadow-md cursor-pointer mb-4 ${bgCard}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-500 mt-0.5">{station.location}</p>
                        </div>
                      </div>
                    </div>
                 ))
               ...
               visibleTrips.length === 0 ? (
                 ...
                 visibleTrips.map(trip => 
                    ...
                    ? <SquadTripCard key={trip.id} trip={trip} user={user} isAdmin={isAdmin} isDarkMode={isDarkMode} setSelectedWeekendTrip={setSelectedWeekendTrip} triggerToast={triggerToast} /> 
                    : <TripCard key={trip.id} trip={trip} user={user} isAdmin={isAdmin} isDarkMode={isDarkMode} openChatFromTrip={openChatFromTrip} triggerToast={triggerToast} />
                 )
               )
             )}
          </div>
      ...
      {viewMode === 'map' && (
         <div className={`text-center py-20 rounded-[2rem] border-2 border-dashed mt-10 ${isDarkMode ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-slate-50'}`}>
             <MapPinned size={48} className="mx-auto text-slate-400 mb-4 opacity-50"/>
             <h3 className="font-bold text-lg text-slate-700 dark:text-slate-300">الخريطة قيد التطوير...</h3>
         </div>
      )}
